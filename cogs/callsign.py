...
# JSON file for logging approved callsigns
CALLSIGN_LOG_FILE = "callsign_approvals.json"
...
def load_callsign_database():
    """Load the callsign database from JSON file"""
    try:
        with open(CALLSIGN_LOG_FILE, 'r') as f:
            return json.load(f)
    except FileNotFoundError:
        return {}
    except json.JSONDecodeError:
        print(f"Warning: {CALLSIGN_LOG_FILE} is corrupted. Starting fresh.")
        return {}


def save_callsign_database(data):
    """Save the callsign database to JSON file"""
    with open(CALLSIGN_LOG_FILE, 'w') as f:
        json.dump(data, f, indent=4)


def check_callsign_exists(callsign: str) -> dict:
    """
    Check if a callsign number is already taken.
    Returns the callsign data if exists, None otherwise.
    """
    database = load_callsign_database()
    return database.get(callsign)


def search_callsign_database(query: str, search_type: str) -> list:
    """
    Search the callsign database by different criteria.
    search_type: 'discord_id', 'roblox_username', 'roblox_id'
    ...
    for callsign, data in database.items():
        if search_type == 'discord_id':
            if str(data.get('discord_user_id')) == query:
                results.append((callsign, data))
        elif search_type == 'roblox_username':
            roblox_user = data.get('roblox_username', '')
            if roblox_user and query_lower in roblox_user.lower():
                results.append((callsign, data))
        elif search_type == 'roblox_id':
            if str(data.get('roblox_user_id')) == query:
                results.append((callsign, data))

    return results


def add_callsign_to_database(callsign: str, discord_user_id: int, discord_username: str,
    ...
    old_callsigns = search_callsign_database(str(discord_user_id), 'discord_id')

    # Store history of previous callsigns
    history = []
    for old_callsign, old_data in old_callsigns:
        # Remove old callsign entry
        if old_callsign in database:
            # Save to history
            history.append({
                "callsign": old_callsign,
                "fenz_prefix": old_data.get("fenz_prefix"),
                "hhstj_prefix": old_data.get("hhstj_prefix"),
                "approved_at": old_data.get("approved_at"),
            ...
            del database[old_callsign]

    # Add new callsign with history
    database[callsign] = {
        ...
        "hhstj_prefix": hhstj_prefix,
        "approved_at": int(datetime.utcnow().timestamp()),
        "approved_by_id": approved_by_id,
        "approved_by_name": approved_by_name,
        "callsign_history": history  # NEW: Store previous callsigns
    }

    save_callsign_database(database)


def remove_callsign_from_database(callsign: str):
    """Remove a callsign from the database"""
    database = load_callsign_database()
    if callsign in database:
        del database[callsign]
        save_callsign_database(database)
